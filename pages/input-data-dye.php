...
$qryDye=sqlsrv_query($con,"SELECT TOP 1 *,CONVERT(VARCHAR(10), tgl_celup, 105) as tgl_celup FROM db_qc.tbl_cocok_warna_dye WHERE [nokk] =? ORDER BY [id] DESC",[$nokk]);
$dtDye=sqlsrv_fetch_array($qryDye,SQLSRV_FETCH_ASSOC);

$qryDye1=sqlsrv_query($con2,"SELECT TOP 1 * FROM db_dying.tbl_hasilcelup WHERE [nokk]=? ORDER BY [id] DESC",[$nokk]);
